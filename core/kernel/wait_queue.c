...
static TEE_Result do_notif(bool sleep, int id, uint32_t timeout_ms,
				       ...
				       int lineno)
{
	...
	do {
		res = do_notif(true, wqe->handle, timeout_ms,
			       "sleep", sync_obj, fname, lineno);

		old_itr_status = cpu_spin_lock_xsave(&wq_spin_lock);

		if (res == TEE_SUCCESS || res == TEE_ERROR_TIMEOUT)
			wqe->done = true;
...
TEE_Result wq_wait_final_timeout(struct wait_queue *wq,
				 struct wait_queue_elem *wqe,
